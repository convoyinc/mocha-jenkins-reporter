...
function Jenkins(runner, options) {
  ...
  options.jenkins_report_suite_duration_discrepancy = process.env.JENKINS_REPORT_SUITE_DURATION_DISCREPANCY || options.jenkins_report_suite_duration_discrepancy;
  ...
  function genSuiteReport() {
    ...
    if (testCount === 0 && currentSuite.duration === 0) {
    ...
    if (options.jenkins_report_suite_duration_discrepancy) {
      // Sometimes the suite duration is larger than the sum of the contained
      // test durations due to time spent in setup/teardown operations.
      // We'll inject a testcase node to represent this difference so that
      // consumers of these results can make more accurate decisions about
      // the duration of a given test file (aka circleci, which seems to
      // only account for testcase times - ignoring testsuite times).
      // We can remove this if/when circle starts taking suite times into
      // account when splitting test files.
      var durationDiscrepancyMs = currentSuite.duration;
      tests.forEach(function(test) {
        durationDiscrepancyMs -= (test.duration || 0);
      });

      if (durationDiscrepancyMs > 0) {
        testSuite.testsuite.push({'testcase': [{
          _attr: {
            classname: "Suite duration discrepancy for " + currentSuite.suite.file,
            name: "(test injected by reporter for capturing setup/teardown time)",
            time: durationDiscrepancyMs/1000,
          }
        }]});
      }
    }
  ...
  function startSuite(suite) {
    // If the suite has a file, we'll track it. The reason we don't make this
    // decision based on number of tests is that if a top-level describe()
    // block has no it()s that are direct descendants, then it has "no tests",
    // even if it contains nested suites with tests. We want to count suites
    // like this because when it has a before() block, we want to count that
    // execution time towards the file.
    if (suite.file) {
      currentSuite = {
        suite: suite,
        tests: [],
