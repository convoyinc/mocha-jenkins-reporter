...
function Jenkins(runner, options) {
  ...
  function genSuiteReport() {
    ...
    if (options.jenkins_report_suite_duration_discrepancy) {
      ...
      if (durationDiscrepancyMs > 0) {
        testSuite.testsuite.push({'testcase': [{
          _attr: {
            classname: "Suite duration discrepancy",
  ...
  runner.on('end', function() {
    endSuite();
    // handle shipotle boot up time
    jsonResults.testsuites.push({'testcase': [{
      _attr: {
        classname: "Shipotle startup time",
        name: "test injected as a placeholder for how long it takes to start shipotle testing",
        time: 10,
        // because we only run one file per process, it's safe to just use the last currentsuite file
        file: currentSuite.file || currentSuite.suite.file
      }
    }]});
    if (fd) {
